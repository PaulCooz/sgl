...
bool graphics::isDeviceSuitable(VkPhysicalDevice device)
{
	...
	return typeDiscrete && extensionsSupported &&
		indices.isComplete() && deviceFeatures.geometryShader && swapChainAdequate;
}
...
void graphics::createSwapChain()
{
	...
	if (hasMaximum && imageCount > swapChainSupport.capabilities.maxImageCount)
	{
...
void graphics::createGraphicsPipeline()
{
	...
	auto res = vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &graphicsPipeline);
	debug::assert(res == VK_SUCCESS, "failed to create graphics pipeline!");
...
void graphics::createFramebuffers()
{
	...
	for (size_t i = 0; i < swapChainImageViews.size(); i++)
	{
		...
		debug::assert
		(
			vkCreateFramebuffer(device, &framebufferInfo, nullptr, &swapChainFramebuffers[i]) == VK_SUCCESS,
...
void graphics::createCommandPool()
{
	...
	debug::assert
	(
		vkCreateCommandPool(device, &poolInfo, nullptr, &commandPool) == VK_SUCCESS,
...
void graphics::createCommandBuffer()
{
	...
	debug::assert
	(
		vkAllocateCommandBuffers(device, &allocInfo, commandBuffers.data()) == VK_SUCCESS,
...
void graphics::createVertexBuffer()
{
	...
	bufferInfo.size = sizeof(vertices.front()) * vertices.size();
...
void graphics::recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex)
{
	...
	debug::assert(vkEndCommandBuffer(commandBuffer) == VK_SUCCESS, "failed to record command buffer!");
}
...
void graphics::drawFrame()
{
	...
	debug::assert
	(
		vkQueueSubmit(graphicsQueue, 1, &submitInfo, inFlightFences[currentFrame]) == VK_SUCCESS,
